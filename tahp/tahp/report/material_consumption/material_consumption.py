...
def get_detailed_data(work_orders, wo_list, prod_item_list, filters):
    ...
    dataset.sort(key=lambda x: (x["consumption_date"] or frappe.utils.nowdate(), x["material_name"]))
...
def get_columns(filters):
    ...
    if is_month_filter:
        ...
        columns = [
            ...
            {"label": _("Nguyên liệu"), "fieldname": "material", "fieldtype": "HTML", "width": 250},
            {"label": _("Đơn vị"), "fieldname": "uom", "fieldtype": "Link", "options": "UOM", "width": 80},
    ...
    if work_orders:
        ...
        for item_code in sorted_prod_items:
            ...
            # Adjust width
            width = 150 if is_month_filter else 150

<template>
  <div>
    <h2 class="tw-text-center tw-text-xl tw-font-semibold tw-mb-4 tw-mt-4">
    ...
    <div class="tw-grid tw-grid-cols-1 lg:tw-grid-cols-5 tw-gap-4">
      ...
      <div
        class="lg:tw-col-span-4 tw-grid tw-grid-cols-1 lg:tw-grid-cols-2 tw-gap-4 tw-mr-4 tw-border tw-border-gray-200 tw-rounded-sm tw-rounded tw-p-4"
      >
        <div class="tw-p-2 tw-border tw-rounded">
          <BaseChart type="bar" :data="barChartData" :options="barChartOptions" />
        </div>
        <div
          class="tw-p-2 tw-border tw-rounded tw-flex tw-flex-col lg:tw-flex-row tw-justify-around tw-items-center tw-gap-4"
        >
          <BaseChart
            type="doughnut"
...
const orderColumns = [
  ...
  { title: "Ngày tạo đơn", key: "createdAt", fieldtype: "Date"  },
  { title: "Thời gian kết thúc dự kiến", key: "expectedEndTime", fieldtype: "Date" },
...
const rows = Array.from({ length: 200 }).map((_, i) => ({
  ...
  status: i % 3 === 0 ? "Đã tạo 1 phần lệnh sản xuất" : "Đã hoàn thành",

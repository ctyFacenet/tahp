  ...
  <div
    ...
    <div class="tw-flex-1 tw-flex tw-flex-col tw-bg-white tw-rounded-xl tw-shadow tw-p-3 sm:tw-p-4 tw-overflow-hidden">
      <div class="tw-flex-col md:tw-flex-row tw-items-start md:tw-items-center tw-justify-between tw-gap-3 tw-mb-3">
        ...
        <div
          class="tw-flex tw-flex-col sm:tw-flex-row tw-items-start sm:tw-items-center tw-justify-between tw-gap-2 tw-w-full">
          <div
...
const onView = (row) =>
  frappe.show_alert({ message: `Đã xem ${row.masterordercode}`, indicator: "green" });
const onEdit = (row) =>
  frappe.show_alert({ message: `Đã chỉnh sửa ${row.masterordercode}`, indicator: "blue" });
const onDelete = (row) => {
  customConfirmModal({
    title: "Xác nhận xoá",
    ...
    buttons: [
      ...
      },
      {
        ...
        class: "btn-danger",
        onClick: () =>
          frappe.show_alert({ message: `Đã xoá ${row.masterordercode}`, indicator: "red" }),
      },
    ],
  });
};

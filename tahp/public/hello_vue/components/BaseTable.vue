    ...
    >
      <table class="tw-min-w-max tw-border-collapse tw-w-full" ref="tableRef">
        ...
        <tbody>
          <template v-if="pagedGroups && pagedGroups.length">
            <template v-for="(group, gIndex) in pagedGroups" :key="group.key">
              <tr class="tw-bg-gray-100 tw-border-b tw-border-gray-300">
                ...
                <td :colspan="columns?.length || 0" class="tw-font-semibold tw-text-red-600 tw-uppercase tw-border tw-px-2">
                  {{ group.key }}
                </td>
              </tr>
          ...
          <tr v-else>
            <td :colspan="(columns?.length || 0) + 2" class="tw-text-center tw-py-6 tw-text-gray-500 tw-italic tw-border">
              Không có dữ liệu
            </td>
          </tr>
        </tbody>

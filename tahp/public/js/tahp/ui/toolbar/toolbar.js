...
// Helper: render template an toàn cho v14/v15
function safe_render_template(template_name, context) {
  // Frappe v14
  if (typeof frappe.render_template === "function") {
    try {
      return frappe.render_template(template_name, context);
    } catch (e) {
      console.warn("render_template lỗi:", e);
    }
  }

  // Frappe v15
  if (typeof frappe.render_template_string === "function") {
    const template = frappe.templates && frappe.templates[template_name];
    if (!template) {
      console.error("Template không tồn tại:", template_name);
      return `<div style="color:red;">Template "${template_name}" not found</div>`;
    }
    return frappe.render_template_string(template, context);
  }

  console.error("Không tìm thấy hàm render template nào trong frappe");
  return "";
}
